<?php
  ...
  if($conn != FALSE)
  {
    ...
    $UID = $_SESSION["uid"];
    $result = mysql_query("select h.h_id as pid,p.Remark as problemName,p.Language as language,p.File_Name as fileName,concat(h.deadlinedate,' ',h.deadlinetime) as deadline,count(su.u_id) as num  ,(case when (select status from submit where su.H_ID = H_id and u_id = '$UID' and status = 'P' limit 1) is null then 'F' else 'P' end)  as status from homework h join section s on h.S_ID = s.S_ID join problem p on p.P_ID = h.P_ID left join submit su on su.H_ID = h.H_ID and su.u_id = '$UID'  where s.S_ID = '$Section' and h.deleteflag is null and (DATE(now())<=h.Deadlinedate or (DATE(now()) = h.deadlinedate and now()<=h.deadlinetime)) and p.deleteflag is null group by h.h_id order by h.h_id;");
    $RowNum = 0;
