<?php
  ...
  if($conn != FALSE)
  {
    ...
    $result1 = mysql_query("select firstname,lastname, concat(firstname,' ',lastname) as name,u.u_id as stdId from user u join register r on r.u_id = u.u_id where r.s_id = '$SID';");
...
//               echo "<i class='fa fa-check' aria-hidden='true' style='color:#2ECC71'></i>";
              echo "<i class='fa fa-check' aria-hidden='true' style='color:#2ECC71' onclick = 'ResultModalHeader($ID,$HidModal,$CountRowForModal,$modalResultHeader,$FN,$LN);' data-toggle='modal' data-target='#modalSourceFileSend'></i>";
            }
            else {
              echo "<i class='fa fa-times' aria-hidden='true' style='color:#E74C3C' onclick = 'ResultModalHeader($ID,$HidModal,$CountRowForModal,$modalResultHeader,$FN,$LN);' data-toggle='modal' data-target='#modalSourceFileSend'></i>";
            }
          echo "</td>";
        }
